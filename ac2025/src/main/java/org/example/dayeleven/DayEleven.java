...
public class DayEleven {
    ...
    public static void main(String[] args) {
        ...
        //  Conecta os dispositivos e as suas saidas Ex: start: path1, path2...
        HashMap<String, ArrayList<String>> graph = dayEleven.createGraph(inputReal);

        // Cache para evitar calcular rotas ja efetuadas anteriormente: dispositivo -> qtd de caminhos possiveis para out
        HashMap<String, Long> cachePossibility = new HashMap<>();

        long totalPaths = dayEleven.countPaths(graph, "you", cachePossibility);

        System.out.println("Result: " + totalPaths);
    }

    HashMap<String, ArrayList<String>> createGraph(String input) {
        ...
        for (String line : lines) {
            ...
            // Separa os valores (dispositivos conectados)
            String[] destinations = parts[1].trim().split(" ");
            graph.put(key, new ArrayList<>(Arrays.asList(destinations)));
        }

        return graph;
    }

    long countPaths(HashMap<String, ArrayList<String>> graph, String currentDevice, HashMap<String, Long> cachedPossibility) {

        if (currentDevice.equals("out")) {
            return 1; // Condição de parada ao encontrar o fim
        }

        if (cachedPossibility.containsKey(currentDevice)) {
            return cachedPossibility.get(currentDevice);
        }

        long possibilityByThisPoint = 0;

        ArrayList<String> nextDevices = graph.getOrDefault(currentDevice, new ArrayList<>());

        for (String nextDevice : nextDevices) {
            possibilityByThisPoint += countPaths(graph, nextDevice, cachedPossibility);
        }

        cachedPossibility.put(currentDevice, possibilityByThisPoint);
