...
public class DayElevenPartTwo {
    ...
    public static void main(String[] args) {
        ...
        DayElevenPartTwo dayElevenPartTwo = new DayElevenPartTwo();

        HashMap<String, ArrayList<String>> graph = dayElevenPartTwo.createGraph(inputReal);

        // 0 -> nenhum, 1 -> dac, 2 -> fft, 3 -> dac + fft
        HashMap<String, Long> cachePossibility = new HashMap<>();

        long totalPaths = dayElevenPartTwo.countPaths(graph, "svr", 0, cachePossibility);
    ...
    private long countPaths(HashMap<String, ArrayList<String>> graph,
                            String current,
                            long finded,
                            HashMap<String, Long> cachePossibility) {

        var cache = cachePossibility.get(current+finded);
        if (cache != null) {
            return cache;
        }
        ...
        for (String next : graph.getOrDefault(current, new ArrayList<>())) {
            total += countPaths(graph, next, finded, cachePossibility);
        }

        cachePossibility.put(current+finded, total);
