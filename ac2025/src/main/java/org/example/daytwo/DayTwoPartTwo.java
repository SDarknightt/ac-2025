...
public class DayTwoPartTwo {
    ...
    public static void main(String[] args) {
        ...
        String[] intervals = inputReal.split(",");
    ...
    void verifyInvalidId(Long id) {
        ...
        if (lengthString % 2 == 0) {
            ...
            if (firstPart.equals(secondPart)) {
                ...
                if (id < 100) return;
                String[] splitedArr = new String[lengthString / 2];
                int index = 0;
                for (int i = 0; i <= lengthString; i++) {
                    if (i % 2 == 0 && i != 0) {
                        if (i == 2) {
                            splitedArr[index] = stringfiedId.substring(0, i);
                            index++;
                            continue;
                        }
                        splitedArr[index] = stringfiedId.substring(i - 2, i);
                        index++;
                    }
                }
                int allEqual = 0;
                for (int i = 0; i < splitedArr.length; i++) {
                    if (splitedArr[0].equals(splitedArr[i])) {
                        allEqual++;
                        continue;
                    }
                }

                if (allEqual == splitedArr.length) {
                    this.counter += id;
                    System.out.println("Invalid: " + id);
                }
            }
            ...
            if (firstPart.equals(secondPart) && secondPart.equals(thirdPart)) {
                System.out.println("Invalid impar: " + id);
                this.counter += id;
            }
        } else {
            if (id < 10) return;
            String[] splitedArr = id.toString().split("");
            int countEqualLetter = 0;
            for (String str : splitedArr) {
                if (str.equals(splitedArr[0])) {
                    countEqualLetter++;
                }
            }
            if (countEqualLetter == splitedArr.length) {
                this.counter += id;
                System.out.println("Invalid all equal: " + id);
            }
        }
    }
